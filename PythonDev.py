    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container and self.ant.can_contain:
                	self.ant.contain_ant(insect)
                elif insect.is_container and insect.can_contain:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if destination == self.place.exit:
        	self.direction = 1
        else:
        	self.direction = 0
        # END EC
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while place != hive:
        	if place.bees and moves >= self.min_range and moves <= self.max_range:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        	...
        	self.true_queen_ant = False
        	if QueenAnt.is_true:
        		self.true_queen_ant = True 
    ...
    def action(self, colony):
        ...
        if self.self_destruct:
        	Insect.reduce_armor(self, self.armor)
       		...
       		while start:
       			if start.ant:
       				if not start.ant.is_doubled:
       					start.ant.damage *= 2
       					start.ant.is_doubled = True
       				if start.ant.is_container:
       					if start.ant.contained_ant:
       						if not start.ant.contained_ant.is_doubled:
       							start.ant.contained_ant.damage *= 2
       							start.ant.contained_ant.is_doubled = True
       			start = start.exit
